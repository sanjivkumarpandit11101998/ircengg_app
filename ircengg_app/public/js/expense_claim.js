...
//     if (doc.expense_type != "Hotel/Room Accommodation")
//         return error;
...
//         doc.amount = rent;
//         if (!doc.change_sanctioned_amount)
//             doc.sanctioned_amount = rent;
...
//     var error = "";
//     if (doc.model_of_travel != "Auto/Taxi")
//         return error;
    
//     /* In case of Auto/taxi, cost is mandatory */
//     if (!doc.per_day_total_cost_of_taxi || doc.per_day_total_cost_of_taxi <= 0)
...
//         var taxi_rent = no_of_days * doc.number_of_taxi * doc.per_day_total_cost_of_taxi;

//         /* Calculate Cost or Taxi */
//         doc.amount = taxi_rent;
//         if (!doc.change_sanctioned_amount)
//             doc.sanctioned_amount = taxi_rent;
//     }
...
//         if (doc.expense_type == travel_expense[i]) {
//             flag = 1;
//             break;
//         }
//     }
...
// 	     */
// 	    var tbl = frm.doc.expenses;
//         var ts = 0; // total sanctioned
//         var ta = 0; // total amount
...
//                     if (tbl[row].expense_type == travel_expense[j]) {
//                         travel_expense_amount += tbl[row].amount;
//                         expensed = 1;
//                         break;
//                     }
//                 }
...
//                 if (!expensed) {
//                     other_expense_amount += tbl[row].amount;
//                 } else
//                     expensed = 0;
//             } else {
//                 error_message += "<br>";
...
//             frm.doc.grand_total = ta;
//             frm.doc.total_sanctioned_amount = ts;
//             frm.doc.total_taxes_and_charges = ta;
//         }
// 	}, /* end of before_save */
...
function validate_hotel_accomodation(idx, doc, tour_start_date, tour_end_date) {
    ...
    if (doc.expense_type != "Hotel/Room Accommodation(with GST)" && doc.expense_type != "Hotel/Room Accommodation(without GST)")
        return error;
...
function validate_travel_end_location(idx, doc) {
    ...
    var travel_expense = ["Travelling", "Ticket Allowance", "Tour Claim", "Conveyance Cost in Delhi"];
    for (var i = 0; i < travel_expense.length; i++) {
        if (doc.expense_type == travel_expense[i]) {
            flag = 1;
            break;
...
frappe.ui.form.on('Expense Claim', {
	before_save	(frm) {
        ...
        var vresponse = "";
	    var row = 0;
        var travel_expense = ["Travelling", "Ticket Allowance", "Tour Claim", "Conveyance Cost in Delhi", "Hotel/Room Accommodation(with GST)", "Hotel/Room Accommodation(without GST)"];
        var travel_expense_amount = 0;
        var other_expense_amount = 0;
        var expensed = 0;
	    // All business logic would sit here
...
frappe.ui.form.on('Expense Claim Detail', {
	expense_type(frm, cdt, cdn) {
	       // if(locals[cdt][cdn].expense_type=='DP Chemical(with GST)' || locals[cdt][cdn].expense_type=='Consumables(with GST)' || locals[cdt][cdn].expense_type=='Safety Items(with GST)' || locals[cdt][cdn].expense_type=='Hotel/Room Accommodation(with GST)' || locals[cdt][cdn].expense_type=='Miscellaneous(with GST)')
            if(locals[cdt][cdn].expense_type.includes('with GST'))
	        {
	            frappe.model.set_value(cdt, cdn, 'bill_is_taxable', 1);
	            frm.refresh_field('expenses');
	        }
	        else
	        {
	            frappe.model.set_value(cdt, cdn, 'bill_is_taxable', 0);
	            frm.refresh_field('expenses');
	        }
		// your code here
	}
//     amount(frm, cdt, cdn){
//         frm.trigger('change_sanctioned_amount')
//     },
// 	per_day_total_cost_of_taxi(frm, cdt, cdn){
// 	    frm.trigger('change_sanctioned_amount')
// 	},
// 	room_rate_per_day(frm, cdt, cdn)
// 	{
// 	    frm.trigger('change_sanctioned_amount')
// 	}
})